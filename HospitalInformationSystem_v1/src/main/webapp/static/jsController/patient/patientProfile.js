								...
								.ajax({
									...
									success : function(data) {
										document.getElementById("doctor").value = data.name
												+ " " + data.surname;
										if (doctor == sessionStorage
												.getItem('person')) {
											document
													.getElementById("recordBtn").style.display = "block";
										} else {
											document
													.getElementById("recordBtn").style.display = "none";
										}
									},
									error : function(e) {
										alert('Error: ' + e);
...
function changePassword() {
	...
	if (newPassword == newRPassword && oldPassword != "" && newPassword != ""
			&& newRPassword != "") {
		$
				.ajax({
					type : "PUT",
					...
					error : function(e) {
						toastr
								.error("Uneli ste pogrešnu staru lozinku. Molimo Vas pokušajte ponovo!");
						$('#oldPassword').val("");
						$('#newPassword').val("");
						$('#newRPassword').val("");
					}
...
function JSONChangeProfile(username, email, country, city, zipCode, street,
		number) {
	return JSON.stringify({
		"username" : username,
		"email" : email,
	...
	if (username == "" || email == "") {
		...
		$
				.ajax({
					...
					contentType : "application/json",
					url : "/patients",
					data : JSONChangeProfile(username, email, country, city,
					...
					error : function(e) {
						toastr
								.error("Uneli ste pogrešne podatke. Molimo Vas pokušajte ponovo!");
					}
				});
	}
}
	...
	if (email != "") {
		$
				.ajax({
					...
					error : function(e) {
						toastr
								.error("Email adresa se već koristi! Unesite drugu adresu.");
					}
				});
	}
}
